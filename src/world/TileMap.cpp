...
TileMap::TileMap() : m_Tiles(c_Width * c_Height)
{
    for (int y = 0; y < c_Height; ++y)
    {
        ...
        {
            auto& tile = m_Tiles[static_cast<std::size_t>(y * c_Width + x)];
            if ((x + y) % 13 == 0)
            {
...
{
    const int tileX = static_cast<int>(worldPosition.x) / c_TileSize;
    const int tileY = static_cast<int>(worldPosition.y) / c_TileSize;
...
TileType TileMap::getTileTypeAt(const engine::math::Vec2i& coord) const
{
    return m_Tiles[static_cast<std::size_t>(coord.y * c_Width + coord.x)].type;
}
} // namespace redclone::world
