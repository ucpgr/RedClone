...
math::Vec2f Camera2D::screenToWorld(const math::Vec2f& screen) const { return ((screen - (m_ViewportSize * 0.5F)) * (1.0F / m_Zoom)) + m_Position; }
math::Vec2f Camera2D::worldToScreen(const math::Vec2f& world) const { return ((world - m_Position) * m_Zoom) + (m_ViewportSize * 0.5F); }
}
