...
bool TileAssetLoader::loadMetadataFile(const std::string& filePath, TileSheetDefinition& out, std::string& error)
{
    ...
    try
    {
        ...
        for (int i = 1; i <= count; ++i)
        {
            ...
            tile.x = reqInt(L, -1, "x"); tile.y = reqInt(L, -1, "y"); tile.w = reqInt(L, -1, "w"); tile.h = reqInt(L, -1, "h");
            if (tile.w <= 0 || tile.h <= 0 || tile.x < 0 || tile.y < 0) throw std::runtime_error("invalid source rectangle for tile: " + tile.name);
            if (tile.kind == TileKind::Flat) { tile.height = reqInt(L, -1, "height"); tile.lowHeight = tile.highHeight = tile.height; }
            else { tile.lowHeight = reqInt(L, -1, "low_height"); tile.highHeight = reqInt(L, -1, "high_height"); if (tile.kind == TileKind::Ramp) tile.direction = parseDirection(reqString(L, -1, "direction")); }
...
bool TileAssetLoader::loadDirectory(const std::string& directoryPath, TileAssetRegistry& out, std::string& error)
{
    ...
    for (const auto& entry : fs::directory_iterator(directoryPath))
    {
        ...
        if (!loadMetadataFile(entry.path().string(), sheet, error)) return false;
        out.registerSheet(std::move(sheet));
    }
    return true;
}
} // namespace redclone::engine::assets
